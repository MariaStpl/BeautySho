  ...
  connection.query(query, [user.email], (err, results) => {
    if (!err) {
      if (results.length <= 0) {
        ...
        connection.query(
          query,
          [user.name, user.contactNumber, user.email, value],
          (err, results) => {
            if (!err) {
...
router.post("/login", (req, res) => {
  const user = req.body;
  query =
    "select id, email, password, contactNumber, role, status from user where email=?";
    ...
    if (!err) {
      if (resultsSelect.length <= 0) {
        ...
        var mailOptions = {
          ...
          html:
            "<p>Recently you requested to reset your Beauty Shop Account.<br>" +
            "<b>Email: </b>" +
            resultsSelect[0].email +
            "<br><b>Token: </b>" +
            resultsSelect[0].reset_token +
            "<br>Click the button below and follow the reset instructions.<br><br>" +
            '<a href="http://localhost:4200/user/resetPassword"><input type=button style="background: linear-gradient(to bottom right, #EF4765, #FF9A5A); color:white; border-radius: 4px; border:none; padding:7px" value="Reset Password"></a></p>',
        };
...
router.patch("/resetPassword", async (req, res) => {
  ...
  connection.query(query, [value, user.reset_token], (err, results) => {
    if (!err) {
      ...
      return res
        .status(200)
        .json({ message: "User create new password Successfuly" });
    } else {
      return res.status(500).json(err);
    }
  });
...
router.get("/getByUser/:userID", auth.authenticateToken, (req, res, next) => {
  const userID = req.params.userID;
  var query = "select id, address from userAddress where userId =?";
...
router.post("/addAddress", auth.authenticateToken, (req, res) => {
  let user = req.body;
